...
- **Resolved Issues**:
  ...
  - Fixed crash in notification settings due to missing database row.
  - Cleaned up git repository by removing AI-specific configuration directories (`.cline*`).
...
- Updated `.gitignore` to prevent AI-specific configuration directories from being tracked.
- Verified Discord notifications are working correctly with the fix.
- Pushed changes to GitHub.
