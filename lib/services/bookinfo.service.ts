...
export class BookinfoService {
   ...
   * Get book by work ID
   */
  static async getBookById(workId: string, signal?: AbortSignal): Promise<Book | null> {
    try {
      const response = await fetch(`${this.apiUrl}/work/${workId}`, { signal })
   ...
   */
  static async getBulkBooks(bookIds: number[], signal?: AbortSignal): Promise<Book[]> {
    const startTime = performance.now()
    try {
