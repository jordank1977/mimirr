...
export class BookshelfService {
  ...
  /**
   * Find author in Bookshelf library by foreignAuthorId
   */
  static async findAuthorInLibrary(
    config: BookshelfConfig,
    foreignAuthorId: string
  ): Promise<any | null> {
    try {
      const baseUrl = config.url.replace(/\/$/, '')
      const url = `${baseUrl}/api/v1/author`
      ...
      if (!response.ok) {
        logger.error('Failed to fetch authors from Bookshelf library', {
          status: response.status,
          statusText: response.statusText,
        })
        return null
      ...
      return authors.find((a: any) => a.foreignAuthorId === foreignAuthorId) || null
    } catch (error) {
      logger.error('Failed to find author in Bookshelf library', {
        error,
        foreignAuthorId,
      })
      return null
  ...
  static async addBook(
    ...
    try {
      ...
      // NEW LOGIC: Check if we are doing "Only This Book" monitoring
      if (bookData.monitoringOption === 'specificBook') {
        // Check if author exists in library
        const existingAuthor = await this.findAuthorInLibrary(config, authorMetadata.foreignAuthorId)
        
        let bookToAdd: any

        if (existingAuthor) {
          logger.info('Author already exists in library, adding book to existing author', {
          ...
          // Construct payload for /api/v1/book with new author
          bookToAdd = {
            title: bookMatch.title,
            foreignBookId: bookMatch.foreignBookId,
            monitored: true,
            ...
            author: {
              name: authorMetadata.authorName,
              foreignAuthorId: authorMetadata.foreignAuthorId,
              monitored: true,
              monitorNewItems: 'none', // CRITICAL: This ensures future books aren't monitored
              ...
              addOptions: {
                ...
                // CRITICAL: monitor must be undefined to avoid auto-monitoring all/future
              },
            },
          }
        }

        logger.info('Adding specific book via /api/v1/book', {
          title: bookMatch.title,
          foreignBookId: bookMatch.foreignBookId,
        ...
        if (!response.ok) {
          const errorText = await response.text()
          logger.error('Failed to add book via /api/v1/book', {
            status: response.status,
            error: errorText,
          })
          return {
          ...
          return {
            success: true,
            bookshelfId: data.authorId, // We typically store the author ID as bookshelfId in requests table
          }
        }
      }

      // Step 4: Add the AUTHOR with specific book monitoring (Default Legacy/Fallback Behavior)
      ...
      const authorToAdd = {
        ...
        rootFolderPath: rootFolderPath,
        addOptions: {
          // CRITICAL: monitor must be undefined to avoid auto-monitoring all/future (do not set to 'none')
      ...
      logger.debug('Adding author to Bookshelf (Legacy Method)', {
