...
// Mood and Pace keywords for metadata harvesting and recommendation scoring
export const MOOD_KEYWORDS = ['Dark', 'Mysterious', 'Uplifting', 'Melancholic', 'Whimsical', 'Tense', 'Funny', 'Romantic'];
export const PACE_KEYWORDS = ['Fast-paced', 'Slow-burn', 'Steady'];
...
/**
 * Calculate mood preferences from user's request history
 * Returns array of {mood, weight} sorted by weight descending
 */
async function calculateMoodPreferences(userId: number): Promise<{ mood: string, weight: number }[]> {
  const requests = await RequestService.getRequestsByUserId(userId)
  ...
  for (const request of requests) {
    ...
    // Extract moods from book genres/tags
    for (const genre of book.genres) {
      if (MOOD_KEYWORDS.includes(genre)) {
        moodScores[genre] = (moodScores[genre] || 0) + timeWeight
      }
    }
  }
  ...
  const preferences = Object.entries(moodScores)
    .map(([mood, score]) => ({ mood, weight: score / maxScore }))
    .sort((a, b) => b.weight - a.weight)
...
/**
 * Calculate pace preferences from user's request history
 * Returns array of {pace, weight} sorted by weight descending
 */
async function calculatePacePreferences(userId: number): Promise<{ pace: string, weight: number }[]> {
  const requests = await RequestService.getRequestsByUserId(userId)
  ...
  const paceScores: { [pace: string]: number } = {}
  ...
  for (const request of requests) {
    ...
    // Extract paces from book genres/tags
    for (const genre of book.genres) {
      if (PACE_KEYWORDS.includes(genre)) {
        paceScores[genre] = (paceScores[genre] || 0) + timeWeight
      }
    }
  }

  // Normalize and sort
  const maxScore = Math.max(...Object.values(paceScores))
  if (maxScore === 0) return []

  const preferences = Object.entries(paceScores)
    .map(([pace, score]) => ({ pace, weight: score / maxScore }))
    .sort((a, b) => b.weight - a.weight)
...
/**
 * Extract top N moods from mood preferences
 */
function extractTopMoods(moodPreferences: { mood: string, weight: number }[], n: number = 3): string[] {
  return moodPreferences.slice(0, n).map(m => m.mood)
}

/**
 * Extract top N paces from pace preferences
 */
function extractTopPaces(pacePreferences: { pace: string, weight: number }[], n: number = 3): string[] {
  return pacePreferences.slice(0, n).map(p => p.pace)
}
...
export async function updateUserPreferences(userId: number): Promise<void> {
  try {
    ...
    const moodPreferences = await calculateMoodPreferences(userId)
    const pacePreferences = await calculatePacePreferences(userId)
    const topGenres = extractTopGenres(genreWeights, 5)
    const topAuthors = extractTopAuthors(authorPreferences, 5)
    const topMoods = extractTopMoods(moodPreferences, 3)
    const topPaces = extractTopPaces(pacePreferences, 3)
    ...
    await db
      ...
      .values({
        ...
        topPaces: JSON.stringify(topPaces),
